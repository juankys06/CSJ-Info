              ...
              <div class="btn-group">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  ...
                  <a
                    ...
                    data-toggle="modal"
                    class="dropdown-item"
                    @click="fieldsUpdate(user.roles, user.id)"
                    >Asignar Rol</a
                  >
                  <a class="dropdown-item" @click="eliminar(user.id, index)"
                    >Eliminar Usuario</a
                  >
                </div>
              </div>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 v-if="!editarRol" class="modal-title">Asignar Rol</h5>
...
export default {
  ...
  mounted() {
    let vm = this;
    axios.get(process.env.VUE_APP_URL + "/users").then(function (response) {
  ...
  methods: {
    ...
    eliminar(id, index) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete(process.env.VUE_APP_URL + "/users/" + id, config)
            .then(() => {
    ...
    Asignar() {
      let vm = this;
      this.isLoading = true;
      this.fields.roles = this.Rol;
      this.fields.header = { Authorization: "Bearer " + Login.methods.token };
      //let vm = this;
      axios
        .post(
          process.env.VUE_APP_URL + "/users/" + this.idUser + "/roles",
          this.fields,
        )
        .then(function () {
          vm.isLoading = false;
    ...
    fieldsUpdate(rolesFields, user) {
      this.idUser = user;

      for (var i = 0; i < Object.keys(rolesFields).length; i++) {
        this.Rol[i] = rolesFields[i].id;
      }
      this.cont++;
    },
    clickCallback: function (pageNum) {
      ...
      axios
        .get(process.env.VUE_APP_URL + "/users?page=" + pageNum)
        .then(function (response) {
          vm.users = response.data.content;
        });
    },
